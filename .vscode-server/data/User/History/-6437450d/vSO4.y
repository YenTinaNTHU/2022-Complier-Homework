...
%type<int_v> statement expression
%type<string_v> type int_type char_type other_type
%type<string_v> idents ident
%type<string_v> scalar_decl
...
%token<string_v> '=' '+' '-' '*' '/' '%'
%token<string_v> ':' ';' ',' '.' '[' ']' '(' ')' '{' '}'
%token<string_v> CONST SIGNED UNSIGNED SHORT LONG LONG_LONG INT CHAR DOUBLE VOID FLOAT STRUCT RETURN

...
%type<string_v> expr statement factor char string
...
%token<string_v> ':' ';' ',' '.' '[' ']' '(' ')' '{' '}'
%token<string_v> CONST SIGNED UNSIGNED SHORT LONG LONGLONG INT CHAR DOUBLE VOID FLOAT STRUCT RETURN LSHIFT RSHIFT STRING
%token<string_v> FOR DO WHILE BREAK CONTINUE
%token<string_v> IF ELSE SWITCH CASE DEFAULT
%token<string_v> CHAR_START CHAR_END ESCAPR_START ESCAPE_CHAR STRING_START STRING_END
...
string
  : STRING { $$ = $1; }
  | string ESCAPR_START ESCAPE_CHAR string
    {
      size_t n = strlen($1) + strlen($2);
      char *str = (char*) malloc(n*sizeof(char));
      strcat(str, $1); strcat(str, $2);
